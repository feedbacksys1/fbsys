...
def feedback(request):
    ...
    if request.method == "POST":
        ...
        if not name or not email or not message_text:
            messages.error(
                request, "Заполните обязательные поля: Имя, Email, Сообщение."
            )
            return render(
                request,
                "feedback.html",
        ...
        try:
            ...
            messages.success(
                request,
                "Ваше сообщение отправлено. Мы рассмотрим его в ближайшее время.",
            )
    ...
    if request.method == "POST":
        if "profile_submit" in request.POST and profile_form.is_valid():
            ...
            prof, _ = Profile.objects.get_or_create(
                user=user, defaults={"role": Role.STUDENT}
            )
...
def _validate_attachment(file):
    ...
    if ext not in REQUEST_ATTACHMENT_EXTENSIONS:
        return (
            False,
            f'Недопустимый тип файла. Разрешены: {", ".join(sorted(REQUEST_ATTACHMENT_EXTENSIONS))}',
        )
...
def cabinet_view(request):
    ...
    if (
        ...
        and "save_request_type" in request.POST
    ):
        curator_request_type_form = CuratorRequestTypeForm(request.POST)
        if curator_request_type_form.is_valid():
            prof, _ = Profile.objects.get_or_create(
                user=request.user, defaults={"role": Role.TEACHER}
            )
            prof.request_type = (
                curator_request_type_form.cleaned_data.get("request_type") or ""
            ).strip()[:200]
    ...
    if request.method == "POST" and request.POST.get("reply_body") is not None:
        ...
        if reply_form.is_valid():
            ...
            if req and role == Role.TEACHER and req.recipient_id == request.user.pk:
                ...
                and req.sender_id == request.user.pk
                and req.thread_opened_at
            ):
                StudentRequestMessage.objects.create(
                    request=req,
                    author=request.user,
    ...
    if (
        request.method == "POST"
        and request.POST.get("new_status")
        and not redirect_detail
    ):
        request_id = request.POST.get("request_id")
        new_status = request.POST.get("new_status")
        if request_id and new_status and new_status in dict(FeedbackStatus.choices):
            try:
                req = StudentRequest.objects.get(
                    pk=int(request_id), recipient=request.user
                )
    ...
    if (
        role == Role.STUDENT
        and request.method == "POST"
        and not request.POST.get("request_id")
    ):
        ...
        if request_form.is_valid():
            ...
            for f in files:
                ...
                RequestAttachment.objects.create(
                    request=req, file=f, original_name=original_name
                )
    ...
    if detail_id:
        ...
        if detail_request:
            thread_messages = list(
                detail_request.thread_messages.select_related("author").order_by(
                    "created_at"
                )
            )
    ...
    if role == Role.STUDENT:
        qs = StudentRequest.objects.filter(sender=request.user).select_related(
            "recipient"
        )
        if filter_status:
    ...
    if role == Role.TEACHER:
        qs = StudentRequest.objects.filter(recipient=request.user).select_related(
            "sender"
        )
        if filter_status:
            qs = qs.filter(status=filter_status)
        ...
        try:
            ...
            initial_type = ""
        context["curator_request_type_form"] = (
            curator_request_type_form
            or CuratorRequestTypeForm(initial={"request_type": initial_type})
        )
...
def download_request_attachment(request, attachment_id):
    ...
    try:
        ...
        response = FileResponse(
            file_handle, as_attachment=True, filename=attachment.original_name
        )
        return response
...
def admin_statistics(request):
    ...
    return render(
        request, "admin_statistics.html", {"stats": stats, "search_q": search_q}
    )


def _get_teacher_stats(search_q=None):
    """Возвращает список словарей со статистикой по кураторам (для страницы и экспорта)."""
    teachers = User.objects.filter(profile__role=Role.TEACHER).order_by(
    ...
    for t in teachers:
        ...
        reviewed = StudentRequest.objects.filter(
            recipient=t, status=FeedbackStatus.REVIEWED
        ).count()
